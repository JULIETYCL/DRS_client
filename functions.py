...
def post_function(access_url_headers=["string"], access_url="string", access_type="s3", aliases=["string"], checksum="string", checksum_type="sha-256", drs_uri=["drs://drs.example.org/314159", "drs://drs.example.org/213512"], contents_id="string", contents_name="string", created_time="2023-04-16T18:56:55.077Z", description="string", mime_type="application/json", name="string", size=0, updated_time="2023-04-16T18:56:55.077Z", version="string"):
    drs_object_register = {
        "access_methods": [
            {
                "access_url": {
                    "headers": access_url_headers,
                    "url": access_url
                },
                "region": "us-east-1",
                "type": access_type
    ...
    response = requests.post(post_url, data=json.dumps(drs_object_register), headers=headers)
...
def get_function(id, expand):
    response = requests.get (f'{get_url}{id}?expand={expand}')
    if response.status_code==200:
...
def delete_function(id):
    response = requests.delete(f'{delete_url}{id}')
    if response.status_code==200:
...
def put_function(id, access_url_headers=["string"], access_url="string", access_type="s3", aliases=["string"], checksum="string", checksum_type="sha-256", drs_uri=["drs://drs.example.org/314159", "drs://drs.example.org/213512"], contents_name="string", created_time="2023-04-16T18:56:55.077Z", description="string", mime_type="application/json", name="string", size=0, updated_time="2023-04-16T18:56:55.077Z", version="string"):
    drs_object_register = {
        "access_methods": [
            {
                "access_url": {
    ...
    response = requests.put(f"{put_url}{id}", data=json.dumps(drs_object_register), headers=headers)
...
def access_function(id, access_id):
    response = requests.get (f'{access_url}{id}/access/{access_id}')
    if response.status_code==200:
        obj_url = response.json()
        print(obj_url)
        return obj_url
    
def post_info_function(contactUrl="mailto:support@example.com", createdAt ="2019-06-04T12:58:19Z", description="This service provides..."):
    service_info={
    ...
    response = requests.post(post_info_url, data=json.dumps(service_info), headers=headers)
...
def delete_access_id(id,access_id):
    response = requests.delete(f'{delete_access_url}{id}/access/{access_id}')
    if response.status_code ==200:
